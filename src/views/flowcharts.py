...
def _apply_incremental_plan(functions_arg_path, model_dir_abs, out_dir):
    ...
    behaviour). Returns the functions file the engine should use."""
    plan_path = os.path.join(model_dir_abs, "incremental_plan.json")
    if not os.path.isfile(plan_path):
        return functions_arg_path
    try:
    ...
    # 1. carry forward baseline flowcharts for NON-impacted files (engine overwrites impacted)
    if base_ver_dir:
        ...
        if os.path.isdir(base_fc):
            ...
            for fn in os.listdir(base_fc):
                if fn.endswith(".json") and fn != "_summary.json":
                    ...
                    carried += 1
            log(f"incremental: carried forward {carried} baseline flowchart file(s)", "flowcharts")
...
def run(model, output_dir, model_dir, config):
    ...
    # Incremental (M2.4b): restrict to impacted files + carry forward the rest.
    functions_arg_path = _apply_incremental_plan(functions_arg_path, model_dir_abs, out_dir)
