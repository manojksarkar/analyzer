...
def _build_unit_diagram(unit_key, unit_info, units_data, functions_data, fid_to_unit, unit_names, internal_scope="project"):
    ...
    for pid in internal_callers:
        ...
        if line:
            lines.append("    " + line.strip())
    main_line = _node_line(this_id)
    if main_line:
        lines.append("    " + main_line.strip())
...
def run(model, output_dir, model_dir, config):
    ...
    internal_scope = ud_cfg.get("internalScope", "project")
    ...
    for i, unit_key in enumerate(sorted(cpp_units), 1):
        ...
        mermaid = _build_unit_diagram(
            unit_key, unit_info, units_data, functions_data, fid_to_unit, unit_names, internal_scope
        )
        if not mermaid:
            continue
