...
# ---------------------------------------------------------------------------
# Export-time description cache (M-B). Struct + unit summaries are pure functions
# of their inputs, but were re-generated by the LLM on EVERY DOCX export (the last
# non-cached Phase-4 cost). Cache them content-addressed (EntityCache) so an
# unchanged struct/unit reuses its description with no LLM call.
# ---------------------------------------------------------------------------
_AUX_DESC_CACHE = None
...
def _cached_desc(config: dict, key_material: str, generate) -> str:
    """Return the cached description for `key_material` (which must fully determine the
    output), else `generate()` it and cache. All cache errors are non-fatal."""
    from llm_core.cache import EntityCache
    cache = h = None
    try:
        cache = _aux_desc_cache(config)
        model = (config.get("llm") or {}).get("defaultModel", "")
        h = EntityCache.compute_hash(f"{key_material}|model={model}")
        hit = cache.get(h, h)
        if hit is not None:
            return hit
    except Exception:
        cache = h = None
    result = generate()
    if cache is not None and h and result:
        try:
            cache.put(h, h, result)
        except Exception:
            pass
    return result
