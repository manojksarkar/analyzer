...
def build_report(stats: Dict[str, Any]) -> List[str]:
    ...
    L.append(f"    Flowcharts: regenerated {fc.get('regenerated', 0):<4} / {fc.get('total', 0):<4} files "
             f"-> carried {fc.get('carried', 0)} ({_pct(fc.get('carried', 0), fc.get('total', 0))})")
    if decision == "incremental":
        L.append(f"    Summaries : {fc.get('regenerated', 0)} impacted file(s) re-summarized; the rest reused")
