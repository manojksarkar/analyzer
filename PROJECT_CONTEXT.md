...
- **M3 — hardening** — *in progress.*
  ...
  - *Remaining:* move/rename orphan cleanup; consolidate `git_ops`/`git_service`; unit-diagram reuse;
...
- **Next concrete step:** **git_ops/git_service consolidation** (tech debt, no behavior change) or
  **unit-diagram incremental reuse** (mirror M3.6) — both low-risk. Flowchart cross-version reuse (M3.7b) is the
  natural extension of M3.7 if the flowchart LLM cost on reverts matters.
