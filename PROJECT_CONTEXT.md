...
- **M3 — hardening** — *in progress.*
  ...
  - *Remaining:* **version-scoped reads** (`?versionId=` on `components`/`functions`/`flowcharts`); move/rename
    polish; file/module summary gating; consolidate `git_ops`/`git_service`. (Recipe-fingerprint invalidation is
    already automatic via the fingerprint; multi-doc zip already shipped in M1.3b.)
