...
> Updated: 2026-07-01 (feat/web-app-api: fix web UI showing raw Mermaid script instead of flowchart images for tall/split flowcharts. Root cause: a tall flowchart PNG is sliced by `src/views/flowcharts.py` into `{stem}_part_K_of_N.png` siblings (and the single `{stem}.png` is not written), but `api/services/doc_render.py::build_render` only probed the single `flowcharts/{unit_prefix}_{safe}.png`. When it was absent, `image_url` came back `None` and the `DocumentInspectorPage` `FlowchartTableView` fell back to rendering `mermaid` text. Fix: added `_resolve_flowchart_pngs(group_dir, base_stem)` (mirrors `docx_exporter._resolve_flowchart_pngs` — slice parts take precedence over the single PNG, sorted by part index) and `_flowchart_entries(...)` (one `flowchart_table` entry per slice: first carries the full label + `Part K of N` + the mermaid fallback, the rest carry `(continued - Part K of N)` and no mermaid). Both the main-function and private-callee flowchart lookups now use these, so `image_url` is populated for split flowcharts and the UI shows images. No frontend change — `FlowchartTableView` already prefers `imageUrl` over `mermaid` and iterates the `flowcharts` array.)
