...
> Updated: 2026-06-23 (feat/api-server branch: model directory data access — `api/services/model_reader.py` added as the single access point for all `model/` JSON files; `api/routes/model.py` adds 12 new endpoints under `/api/v1/model` (status, metadata, components, units, functions, globals, dictionary, summaries, cache refresh); `JsonDatabase` startup now calls `ModelReader.load_pipeline_functions()` instead of its own inline loader; `document_renderer.build_document_structure()` uses `ModelReader` properties instead of scattered `_load_model_file()` calls; `bad_request()` in `errors.py` extended to accept optional (code, msg) two-arg form; total endpoint count raised from 51 → 63; see §19).
...
> - §19 covers the API server (`api/`): architecture, endpoints, JSON DB adapter, structured document renderer, and the new `ModelReader` + `/api/v1/model` route group.
...
- **JSON DB:** `API_DB_BACKEND=json` persists state to `api/db/data/*.json` and automatically loads pipeline functions from `model/` on startup via `ModelReader`
- **63 endpoints** across auth, projects, commits/versions, analysis jobs, documents, team, compare, functions, notifications, and model data (`/api/v1/model/*`)
- **Model data access:** `GET /api/v1/model/*` exposes all `model/` pipeline output (metadata, components, units, functions, globals, data dictionary, summaries) directly over the API; `POST /api/v1/model/refresh` reloads the cache after a pipeline run

See [`api/PROJECT_CONTEXT.md`](api/PROJECT_CONTEXT.md) for architecture decisions, known issues, seed data, SSE streaming, error envelope, the full route list, JSON DB adapter details (§11), structured document renderer (§12), and `ModelReader` + model routes (§13).
