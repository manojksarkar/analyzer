...
- **M3 — hardening** — *in progress.*
  ...
  - *Remaining:* unit-diagram reuse (low value — no LLM); **M4.1+ narrowed parse** (deferred — doc 04 §11).
    (Recipe-fingerprint invalidation **dropped by decision** — fingerprint is content-only; multi-doc zip shipped in M1.3b.)
- **Next concrete step:** the core incremental feature is **complete** (engine + same-version reuse + cross-version
  reuse for all artifacts + version-scoped reads + branch/commit endpoints + orphan cleanup + one git layer +
  virtual-dispatch over-approximation). Only optional items remain: unit-diagram reuse (low value — no LLM) and
  **M4 narrowed parse** (deferred until Phase-1 parse is the measured bottleneck — fully specced in doc 04 §11).
