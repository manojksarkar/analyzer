...
- **M2 — incremental engine** — *in progress.*
  ...
  - **M2.4** (next) — wire `mode:"auto"` into `POST /generate` (preview decides incremental vs full → dispatch to
    `generate_incremental`/`generate_full`) + **flowchart-level reuse** (restrict the flowchart engine's input to
    the impact set, carry forward the rest) so LLM-on runs skip flowchart labeling for the reuse set.
...
- **Next concrete step:** **M2.4** — wire `mode:"auto"` into backend `POST /generate` (use `generate/preview`'s
  decision to dispatch `generate_incremental` vs `generate_full`; spawn `src/incremental/engine.py` as the job) +
  **flowchart-level reuse** (restrict the flowchart engine's input file to the impact set + carry forward the rest)
  so LLM-on runs skip flowchart labeling for reused functions. (M2.1+M2.2+M2.3 done — engine works e2e; M3 after.)
