...
- **M3 — hardening** — *in progress.*
  ...
  - *Remaining:* move/rename re-resolution + deletions polish; **version-scoped reads** (`?versionId=` on
    `components`/`functions`/`flowcharts`); recipe-fingerprint invalidation; multi-doc zip; consolidate
    `git_ops`/`git_service`.
