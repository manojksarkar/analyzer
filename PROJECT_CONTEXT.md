...
- **M3 — hardening** — *in progress.*
  ...
  - *Remaining:* consolidate `git_ops`/`git_service`; unit-diagram reuse (low value — no LLM);
    virtual/indirect-call over-approximation audit; flowchart cross-version reuse (M3.7b); **M4.1+ narrowed parse**
    (deferred — doc 04 §11).
    (Recipe-fingerprint invalidation **dropped by decision** — fingerprint is content-only; multi-doc zip shipped in M1.3b.)
- **Next concrete step:** **git_ops/git_service consolidation** (tech debt, no behavior change) — the cleanest
  remaining item. Then flowchart cross-version reuse (M3.7b) if reverts' flowchart LLM cost matters.
  (Unit-diagram reuse deprioritized: no LLM, only saves PNG-render time.)
