...
- **M3 — hardening** — *in progress.*
  ...
  - *Remaining:* **cross-version reuse-index lookup** (D3 / §5 step 6 — index seeded but never read; the reuse payoff);
    move/rename orphan cleanup; consolidate `git_ops`/`git_service`; unit-diagram reuse; virtual/indirect-call
    over-approximation audit; **M4.1+ narrowed parse** (deferred — doc 04 §11).
    (Recipe-fingerprint invalidation **dropped by decision** — fingerprint is content-only; multi-doc zip shipped in M1.3b.)
- **Next concrete step:** **M3.7 — cross-version reuse-index lookup** (D3) — make the engine *read*
  `cache/index.json` so reverts / cross-branch-identical entities are copied instead of LLM-regenerated.
  Then move/rename cleanup + git consolidation.
