...
- Functions: `Data Type` = `; `.join of param types (or `VOID` if none), then a
  second line `return: <returnType>` (`VOID` for void). The `\n` renders as a Word
  line break in DOCX and needs `whitespace-pre-line` on the web cell
  (`DocumentInspectorPage.tsx`); the compare/diff view flattens it to a space via
  `_table_to_markdown`. `Data Range` = `; `.join of param ranges from `get_range()`
  (return range not shown — deferred).
