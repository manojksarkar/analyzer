...
- **M3 — hardening** — *in progress.*
  ...
  - *Remaining:* unit-diagram reuse (low value — no LLM); virtual/indirect-call over-approximation audit;
    **M4.1+ narrowed parse** (deferred — doc 04 §11).
    (Recipe-fingerprint invalidation **dropped by decision** — fingerprint is content-only; multi-doc zip shipped in M1.3b.)
- **Next concrete step:** **virtual/indirect-call over-approximation audit** (correctness — confirm a changed
  virtual override impacts callers dispatching through the base; impact.py only over-approximates global/type/macro
  today). Then optionally unit-diagram reuse (low value — no LLM) or M4 narrowed parse (deferred).
