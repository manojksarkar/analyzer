...
- **M3 — hardening** — *in progress.*
  ...
  - *Remaining:* unit-diagram reuse (low value — no LLM); virtual/indirect-call over-approximation audit;
    flowchart cross-version reuse (M3.7b); **M4.1+ narrowed parse** (deferred — doc 04 §11).
    (Recipe-fingerprint invalidation **dropped by decision** — fingerprint is content-only; multi-doc zip shipped in M1.3b.)
- **Next concrete step:** **flowchart cross-version reuse (M3.7b)** — extend the M3.6 splice to pull an
  index-reused (revert) function's flowchart from its source version instead of regenerating. Or the
  **virtual/indirect-call over-approximation audit** (correctness). (Unit-diagram reuse deprioritized: no LLM.)
