...
| S3-9 | `union U {...};` and `using T = int;` reach the data dictionary NEVER -- `UNION_DECL` and `TYPE_ALIAS_DECL` are unhandled cursor kinds -- so they appear in no table at any access level. A `typedef union {...} U;` does appear, through its typedef | issue | open | engine/parser.py, Risk 8 |
...
| S3-11 | doccheck attributes a header row only WE have to the N.1.4 rule by looking for PRIVATE / PROTECTED / STATIC / :: in the declaration text -- but the declaration no longer CONTAINS a marking (they are stripped for display), so that signal is gone for a macro-marked global. The :: and static checks still fire. Simplest fix: attribute any header row we have and they do not, at INFO | issue | open | tools/doccheck/rules.py |
