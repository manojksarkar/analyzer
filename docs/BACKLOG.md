...
| RV-4 | An orphan-header enum appears in every unit that uses the header | decide | Confirmed (2026-09-23): it is listed in every unit that really uses it (no false match), and it should be listed once, in a unit header table. The same rule applies to every kind from an orphan header: `#define`, enum, typedef, struct/class/union and globals. ❓ Which unit header table | views/unit_headers.py, SWE3_WIKI "Orphan-header symbols" |
