...
| 3 | **V1 fixes** | **~14–38** | V1 | P0 | rollup ↑: pre-V1 batch (3.1–3.10) + second review batch (3.11–3.19) + third unit-header-value batch (3.20–3.22). **Status 2026-07-20: DONE + removed from list — 3.1–3.7, 3.14, 3.15, 3.18, 3.20, 3.22, flowcharts-in-DOCX. Remaining: 3.8/3.9 rendering, 3.10 blocked, per-layer macros; 3.11/3.12/3.13/3.16/3.19 second-batch; 3.17 interim (full spec pending); 3.21 partial (comment-strip done, eval/description deferred); 3.23 conditional `#define` both-branches DONE (libclang active-branch selection, parser change → snapshot regen).** |
...
| | **↳ Pre-V1 correctness batch — remaining (3.1–3.7 done + removed; roots + interface-direction landed):** | **~2.5–7** | V1 | P0 | 3.8/3.9 = rendering polish, 3.10 = blocked (repro). |
...
| | **↳ Second correctness batch — remaining (3.11–3.19; 3.14/3.15/3.18 done + removed):** | **~3.5–10** | V1 | P1 | User-confirmed 2026-07-19. Final priority/milestone TBD with manager. Verified against code where noted. 3.17 interim-landed (full spec pending). |
...
| | **↳ Third correctness batch — unit-header value column (3.20–3.22; from 2026-07-20 review):** | **~1–2** | V1 | P1 | All in the unit-header table's 2nd column. **3.20 + 3.22 done + removed** (2026-07-20). Only 3.21's evaluation/description half remains. |
| | ↳ 3.21 #define value — derive/evaluate the value, else show a description | 1–2 | V1 | P1 | ⏳ **partial.** Comment-strip half is **done** (value now shows the clean literal, e.g. `(24)` not `(24) //cmt`). **Remaining (deferred to avoid new-issue risk):** (a) *evaluate* expressions (`(1<<6)`→`64`, resolve nested macros) and (b) description fallback for value-less/function-like macros (currently `NA`). Both add risk (arithmetic/macro eval, LLM surface); do only if the client asks. |
