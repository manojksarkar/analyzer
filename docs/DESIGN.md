...
**Model (single source of truth):** functions.json, globalVariables.json, units.json, modules.json, dataDictionary.json, metadata.json, knowledge_base.json. See [Model Format](#model-format) below.
...
3. **Visibility detection:** `_detect_visibility(file, line)` scans raw source up to 5 lines backwards from the declaration line to detect `PRIVATE`, `PUBLIC`, or `PROTECTED` macro prefix — stored lowercase in `visibility` field (`"default"` if none found).
4. **Comment extraction:** `_preceding_comment(cursor)` reads `//` or `/* */` lines immediately above a function/type definition. `_inline_comment(cursor)` reads trailing `//` comment on struct fields or enum constants.
5. Write `metadata.json`, `functions.json` (with `comment` + `visibility`), `globalVariables.json`, `dataDictionary.json`.
...
Loads model from `model/` then calls each enabled view in `config.views`. **Visibility filtering per view:**
...
- `flowcharts`, `unitDiagrams`, `moduleStaticDiagram`: no visibility filtering

**Case-insensitive module matching:** `_analyzerAllowedModules` is normalised to lowercase; all module comparisons use `.lower()` throughout.
