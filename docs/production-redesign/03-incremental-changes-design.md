...
## 12. Parse-scoping strategies — Approach 1 vs Approach 2 (v1 decision)

> Added in v1.1. **Both approaches implement the same incremental core** (§4–§7:
> detect-by-hash → impact analysis → selective LLM regeneration → reassemble).
> They differ on exactly **one axis: how much of the target commit we re-parse.**
> The expensive part — reusing LLM output instead of regenerating it — is identical
> in both. This section records the comparison and the v1 decision.

### 12.1 The two approaches

**Approach 1 — full-parse + content-addressed reuse (branch-agnostic).**
Re-parse the *entire* target commit every time. For each entity, look up its
**fingerprint** (token-hash of its own source + the hashes of its dependency
closure + the recipe fingerprint) in a **project-wide cache that spans every
generation ever run**: hit → reuse the stored metadata; miss → regenerate it.
Impact is implicit in the fingerprint — a changed dependency changes the
dependent's fingerprint. Git history is never consulted; reuse is purely
content-driven. *(Equivalently: detect the changed set by source-hash, then
reverse-BFS impact — same result; the fingerprint is just the compact encoding.)*

**Approach 2 — git-diff narrowed parse (ancestor-based).**
Store the commit id with each generated document. For a new target commit, find
the **nearest generated git ancestor**, run `git diff <ancestor>..<target>
--name-only` to get the **changed files**, and re-parse only those. Reuse the
baseline version's stored model + metadata for every unchanged file; hash the
entities in the changed files to find the changed set; run impact analysis over
the stored graph; regenerate the impact set.

### 12.2 Do they both work? Yes — but each has non-obvious requirements

Both are correct **only if** the following hold. These are exactly what a naïve
implementation would miss and then silently emit a **stale** document.

**Approach 1 requirements**
1. **Full-parse the target commit** — needed to rebuild the complete *current*
   dependency graph (cheap relative to the LLM; inherent to the approach).
2. **Key the reuse cache by _fingerprint_, never by bare source hash.** The same
   function source can have *different correct* metadata depending on its callees
   (e.g. `a()` is unchanged but `b()` it calls changed → `a()`'s description is now
   stale). Source-hash keying would return the stale entry; the fingerprint
   (source + dependency-closure hashes + recipe) distinguishes the two contexts.
   **← the subtle trap in Approach 1.**
3. **Complete / over-approximated dependency graph** for impact — virtual calls →
   all overrides, function-pointer calls → any address-taken function (§5.3).
4. **Recipe fingerprint** in the key (LLM model + prompt version + cacheVersion) so
   an operator recipe change invalidates everything (§10).

**Approach 2 requirements**
1. **Persist the full dependency graph (edges) per baseline, and run impact
   analysis over the _stored_ graph** — not just over edges inside the changed
   files. Otherwise a changed `b()` in a changed file never reaches its caller
   `a()` living in an *unchanged* file → `a()` stays stale. **← the critical trap
   in Approach 2.**
2. **Move/rename edge re-resolution.** When a callee's key changes (it moved files)
   but the caller sits in an unchanged file, the caller's stored edge is now
   dangling; the caller must be impacted *and* its edges re-resolved (in practice:
   re-parse the impacted caller's file), pushing the parse set beyond git-diff's
   file list.
3. **Baseline selection + full-regen fallback** — pick the nearest generated
   ancestor; when none exists (first generation, diverged/rebased branch) fall back
   to a full generation.
4. **(For maximal reuse) also consult a project-wide content cache** — otherwise a
   revert, or identical code on another branch, regenerates content already
   computed in an earlier version, because it sits in a "changed file" relative to
   the chosen baseline.
5. Plus #3 and #4 of Approach 1 (graph completeness, recipe fingerprint).

Net: both work; **Approach 2 has more sharp edges**, and each edge that is missed
produces the one outcome we must never produce — a stale document.

### 12.3 Advantages / disadvantages

| | **Approach 1 (full-parse + content cache)** | **Approach 2 (git-diff narrowed)** |
|---|---|---|
| Correctness robustness | **Higher** — always rebuilds the true current graph; immune to the move/rename edge-reuse trap | Lower — correct only with stored-graph impact + edge re-resolution |
| Implementation complexity | **Low** — one content cache + impact; no baseline/ancestor bookkeeping | High — stored edges, baseline selection, fallback, re-resolution |
| Reuse breadth (LLM savings) | **Maximal** — reuses content seen in *any* prior version | Baseline-only unless augmented with a global content cache |
| Branch handling | Branch-agnostic by construction | Needs git-ancestry bookkeeping per version |
| Parse cost per run | **Full parse every time** — paid even for a 1-line change | **Narrow** — only changed files; fastest for small diffs at scale |
| Fit for firmware scale (≈50k fns) | Parse may dominate a tiny-change run | Wins exactly here — seconds of parse for a small change |
| Fit for storage backend | Clean in JSON now | Stored edges + ancestor queries map naturally to Postgres recursive CTEs |

The single real win of Approach 2 is **parse time on small changes at scale**; its
cost is **complexity plus several correctness pitfalls**. Approach 1 pays a **full
parse** but is **simpler, more robust, and reuses more**.

### 12.4 Worked example

Call graph (arrows = "calls"):
...
v0 already generated at `main@C0`; the cache holds metadata for every function.
Now regenerate for `feature@C1`, where **only `c`'s body changed** (`c → c'`).

```
            APPROACH 1 (full-parse C1)            APPROACH 2 (diff C0..C1)
            ────────────────────────────          ────────────────────────────
 parse:     ALL files                              only c's file (from git diff)
 detect:    fingerprint miss on c                  hash(c) != stored hash(c)
 impact:    fp also moves for everything           reverse-walk STORED edges:
            whose closure contains c:                c <- b <- a <- main
              c, b, a, main, z                       c <- z
 regen LLM: { c, b, a, main, z }                   { c, b, a, main, z }   <- identical
 reuse:     { d } (+ all untouched)  <- fp hit     { d } (+ all unchanged files)
 result:    SAME document                          SAME document
```

Both produce the identical regeneration set `{c, b, a, main, z}` and reuse `{d}`.
The *only* difference: Approach 1 parsed every file; Approach 2 parsed one.

**Where they diverge — a revert / branch-hop.** Suppose v1 above is stored, and we
now generate `other@C2` whose content is byte-identical to the original `C0`
(`c'` reverted back to `c`):

```
 Approach 1: every fingerprint hits the cache (c was already generated in v0)
             -> 0 LLM calls, near-instant document.
 Approach 2: nearest ancestor is C1; git diff C1..C2 shows c reverted ->
             c is "changed" -> re-runs the LLM for c (+ impact), UNLESS a
             global content cache is also consulted (requirement 12.2 / A2 #4).
```

This is the content-cache reuse advantage of Approach 1: identical content is
*never* regenerated, regardless of branch or commit path.

### 12.5 Decision (v1)

**Adopt Approach 2 for v1** — git-diff narrowed parse (implemented over JSON files
now, Postgres later). Chosen deliberately to **avoid a full re-parse on every
regeneration**: at firmware scale (≈50k functions) Approach 1 pays a full libclang
parse on every run even for a one-line change, and the product is expected to run
many small regenerations.

**Approach 1's full-parse is retained as the _fallback path within_ Approach 2** —
used for the first generation, when no usable nearest-ancestor exists, and on
diverged / rebased history. The two are not exclusive: Approach 2 is the fast path,
Approach 1 is the safety net.

**The §12.2 Approach-2 requirements are mandatory, not optional** — they are the
difference between a correct and a stale document:
- impact analysis MUST run over the **stored full dependency graph** (so impact
  reaches callers that live in unchanged files);
- **move/rename** must trigger caller re-resolution (re-parse the impacted callers);
- **baseline selection** = nearest generated ancestor via `git merge-base`, with a
  **full-regen fallback**;
- a **project-wide content cache** as the cross-version reuse safety net (so a
  revert / identical code on another branch is not needlessly regenerated);
- a **recipe fingerprint** for operator-side invalidation (LLM model / prompts /
  cacheVersion).

> **One-line rule:** v1 = **Approach 2** (git-diff narrowed parse + stored-graph
> impact + selective regen), with **Approach 1's full parse as the fallback** for
> first-generation / no-ancestor / diverged cases.
...
> **v1 scope note (added v1.1, updated v1.2):** §12 records the decision to adopt
> **Approach 2** (git-diff narrowed parse) for v1 — the `git merge-base` / `git
> diff` mechanics described above. **Approach 1's full parse is the fallback** for
> first-generation / no-ancestor / diverged cases.
