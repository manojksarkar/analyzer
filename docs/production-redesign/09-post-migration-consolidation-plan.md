...
| ~~**C3**~~ | ~~Delete `clang_include_paths.json`~~ — **investigated 2026-08-16: the premise is wrong, no work needed.** It is not stored state: `run.py` rewrites it at the start of EVERY run, and it appears nowhere in `generate.py`, `model_store.py`, `_DUMP_FILES` or `_PARSE_SNAPSHOT_FILES` — so it never reaches the database, a version snapshot, or another machine. Its machine-specific paths are therefore always the paths of the machine that just wrote them. It exists to cache an expensive filesystem walk ("so Phase 1 and Phase 3 can read them without re-walking"), so "derive it" would make every run slower for zero correctness gain. Only real nit: it sits in `model/`, which is otherwise DB-backed data — C11c must skip it rather than delete it. | — | **closed** |
| **C4** | **Remove `knowledge_base.json` (D-12 "no knowledge base").** Still referenced in `core/model_io.py`, `flowchart/pkb/builder.py`, `model_deriver.py`. Replace with the context service. **Note (2026-08-16): NOT a migration blocker.** It is a Phase 2 -> Phase 3 hand-off *within one run*, written and consumed on the same machine, so it is not cross-machine state. The only gap it leaves is a cross-machine `--from-phase 3` (see IN-5), which degrades label context rather than failing. Its value is the D-12 memory argument (whole-project KB objects), not portability. | D-12 | M |
