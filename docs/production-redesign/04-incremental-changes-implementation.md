...
- **M3 — Hardening.** Move/rename re-resolution, deletions, over-approximation polish, version-scoped
  ...
  M3.8 branch/commit endpoints + M3.9 version-scoped reads done; remaining: git layer consolidation,
  unit-diagram reuse, move/rename orphan cleanup, flowchart cross-version reuse (M3.7 follow-on — reused
