...
**Visibility filtering:** Private entries are excluded from the interface table, unit header table, unit diagrams, behaviour diagrams and the SWE.4 specs. `PROTECTED` counts as private. A `PUBLIC` marking does not by itself publish a function — since 2026-09-20 a marking may restrict but never promote, so publication is earned by a caller in ANOTHER UNIT (`Foo.h` + `Foo.cpp` count as one, so a companion-header caller publishes nothing) or by a file-scope dispatch table. Globals have no call graph, so their marking alone decides. The DOCX flowchart section renders a non-private function's own flowchart followed by those of any private functions it calls, each at most once per unit (`rendered_private_fids`).
