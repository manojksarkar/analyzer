...
`3)`. Only **Test Steps** nests, because it follows the control flow, and its levels **alternate numeric and
alphabetic** — `2.a)`, `2.a.3)`, `2.a.3.d)` — which stays readable at depth where `2.1.3.4)` does not. An
alphabetic level carries past `z` the spreadsheet way (`z`, `aa`, `ab`). The other three lists stay flat —
where an entry has several values, they are separated by commas on the same line.
