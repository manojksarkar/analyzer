...
# The PUBLIC/PRIVATE/PROTECTED/__OVLYINIT markings expand to nothing -- they reach libclang
# as `-DPUBLIC=`, so the compiler never sees a token. They are dropped from the declaration
# cell: build scaffolding does not belong in a design document, and in THIS table the word
# misleads, since visibility stopped filtering it. Real C++ keywords stay.
class TestVisibilityMacrosStripped:
    def _row(self, decl, value="0"):
        gid = "Comp/UserUnit.cpp:60"
        globals_data = {gid: {
            "variableId": gid, "variableName": "g_x", "qualifiedName": decl,
            "visibility": "public", "value": value,
            "location": {"file": "Comp/UserUnit.cpp", "line": 60},
        }}
        unit = {"path": "Comp/UserUnit", "fileName": "UserUnit.cpp",
                "functionIds": [USER_FID], "globalVariableIds": [gid]}
        rows = dx.build_rows(unit, [], {}, globals_data, "", None, {},
                             {}, {}, SRC_PATHS, None, {})
        return _decls(rows)[0]

    def test_public_marking_dropped(self):
        assert self._row("PUBLIC int g_result = 0;") == "int g_result = 0;"

    def test_private_marking_dropped(self):
        assert self._row("PRIVATE int g_count = 0;") == "int g_count = 0;"

    def test_protected_marking_dropped(self):
        assert self._row("PROTECTED int g_mark = 1;") == "int g_mark = 1;"

    def test_extern_is_kept(self):
        """It says the unit uses a global it does not own -- information nothing else carries."""
        assert self._row("extern int g_sharedTick;") == "extern int g_sharedTick;"

    def test_static_and_const_are_kept(self):
        decl = "static const OpsFn g_opsTable[] = { opsAdd };"
        assert self._row(decl) == decl

    def test_a_qualified_name_survives(self):
        decl = "int NestedOwner::s_privateSecret = 0;"
        assert self._row(decl) == decl
