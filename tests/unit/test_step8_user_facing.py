...
class TestUseModel:
    ...
    def test_message_names_where_it_looked(self):
        """"model files missing" is wrong and unhelpful when the model is in a table."""
        src = _src(os.path.join("engine", "run.py"))
        assert "the model is missing from the database" in src


class TestReexport:
    """Phase 4 finds the model by version id. It used to ASK whether the model was persisted
    and pass the id only if so, because a version generated before the DB work had files
    instead; with no file model that question has one answer."""

    def test_tells_the_phases_which_version_to_re_export(self):
        src = _src(os.path.join("api", "services", "pipeline_runner.py"))
        assert 'version_id=getattr(job, "version_id", None)' in src

    def test_build_cmd_emits_the_version(self):
        ...
        assert "--model-store" not in cmd, "the flag is gone; the database is the only backing"
