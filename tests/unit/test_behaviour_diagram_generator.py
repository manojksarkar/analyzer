...
No LLM required — three classes test the current static implementation:
  TestExternalCallerFiltering  caller filtering logic (pure)
  TestFileNaming               .mmd file naming conventions (pure)
  TestMmdContent               .mmd file content — valid Mermaid without LLM

One class requires LLM (all xfail until the seam is wired):
  TestLlmContract              LLM response written, fences stripped, fallback on empty
...
# One external caller (App module) calling Core|Core|compute|
_ONE_EXTERNAL_CALLER = {
    "Core|Core|compute|": {
...
class TestFileNaming:
    def test_files_use_double_underscore_separator(self, tmp_path):
        gen = _make_generator(tmp_path, _ONE_EXTERNAL_CALLER)
        paths = gen.generate_all_diagrams("Core|Core|compute|", str(tmp_path / "bd"))
        assert paths
