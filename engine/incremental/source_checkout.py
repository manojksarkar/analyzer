...
def _version_row(project_id: str, version_id: str, *, need_commit: bool = True):
    ...
    with get_engine().connect() as cx:
        ...
        if not row.commit_sha and need_commit:
            known = [r.id for r in cx.execute(
                sa.select(s.versions.c.id).where(s.versions.c.project_id == project_id,
                                                 s.versions.c.commit_sha.isnot(None)))]
...
def locate_or_restore(project_id: str, version_id: str, *,
                      ...
                      commit: Optional[str] = None) -> SourceCheckout:
    """The checkout `version_id` was generated from, restoring it if nothing usable is on disk.

    `commit` names the commit to find when the caller knows better than the `versions` column --
    `analyzer.py reexport` reads the version's manifest, which records what the run actually built
    from; the column can be stale.
    """
    row = _version_row(project_id, version_id, need_commit=not commit)
    commit = commit or row.commit_sha
