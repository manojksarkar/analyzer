...
def run(model, output_dir, model_dir, config):
    ...
    # poc-4 parses every layer into one flat model, so functions/globals/dataDictionary
    # are already loaded in full here — no per-layer files to re-read.
    all_functions = model.get("functions", {})
    global_vars = model.get("globalVariables", {})
    ...
    for layer_name, layer in layers.items():
        ...
        callers_of, callees_of = _build_call_graph(layer_comp_names, all_functions)
        ...
        for grp_name, comps_cfg in groups_cfg.items():
            for comp_name in comps_cfg:
                ...
                interfaces = _build_interfaces_for_component(
                    ...
                    units_data, all_functions, global_vars, dd, comp_to_group,
                )
