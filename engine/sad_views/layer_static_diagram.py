...
def run(model, output_dir, model_dir, config):
    ...
    # Load component summaries once from the single flat knowledge_base.json.
    # Keys are still layer-prefixed (e.g. "Layer2/Platform/Adc") because
    # location.file in the model is relative to the project root, which
    # already includes the layer segment.
    kb_path = os.path.join(model_dir or _paths().model_dir, "knowledge_base.json")
    comp_summaries: dict = {}
