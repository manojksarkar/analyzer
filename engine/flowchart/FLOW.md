...
Starting point: `python engine/run.py <project_path>`
...
_run_pipeline(from_ph)
  subprocess: python engine/parser.py        <project_path>   → Phase 1
  subprocess: python engine/model_deriver.py                  → Phase 2
  subprocess: python engine/run_views.py                      → Phase 3
  subprocess: python engine/docx_exporter.py                  → Phase 4
  each phase must exit 0, else pipeline stops
```

---

## PHASE 1 — C++ Source Parser (`engine/parser.py`)
...
## PHASE 2 — Model Deriver (`engine/model_deriver.py`)
...
model_deriver.py  main()
  ...
  │                                                                    │
  │     Imports from engine/flowchart/ (reuses flowchart module):         │
...
## PHASE 3 — Views Generator (`engine/run_views.py`)
...
### Flowcharts View — `engine/views/flowcharts.py`

```
flowcharts.run(model, output_dir, model_dir, config)
  reads config.views.flowcharts.scriptPath
      → resolves to engine/flowchart/flowchart_engine.py
...
### `engine/flowchart/flowchart_engine.py`
...
### `engine/flowchart/llm/generator.py`
...
## PHASE 4 — DOCX Exporter (`engine/docx_exporter.py`)
