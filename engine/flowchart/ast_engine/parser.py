...
class TranslationUnitParser:
    ...
    def _cached(self, key: str, build) -> ci.TranslationUnit:
        ...
        CC-2: `flowchart_engine.py`'s run() parallelizes `_process_function` over the function
        list, so multiple threads can ask for the same (or a different) TU concurrently.
        libclang's C API gives no documented guarantee that concurrent
        `clang_parseTranslationUnit` calls against one shared `CXIndex` are safe, so the whole
        get-or-build-and-evict sequence — not just the cache dict update — is serialized
        through `self._lock`. This only serializes the libclang parse itself (fast,
        CPU-bound); the LLM label calls that dominate wall-clock still run concurrently
        outside this lock.
        """
        with self._lock:
