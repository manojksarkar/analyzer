...
from ..schemas import NotificationListResponse, NotificationResponse, MessageResponse
...
@router.get("", responses={200: {"model": NotificationListResponse}})
...
@router.patch("/{notification_id}/read", responses={200: {"model": NotificationResponse}})
def mark_read(
    notification_id: str,
    current_user: User = Depends(get_current_user),
    db: InMemoryDatabase = Depends(get_db),
...
@router.post("/read-all", responses={200: {"model": MessageResponse}})
def mark_all_read(
    current_user: User = Depends(get_current_user),
    db: InMemoryDatabase = Depends(get_db),
):
